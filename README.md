...
LLM CLIを使って、ツールやプロンプトの**出力（成果物）の質を螺旋状に最適化**するための最小CLIです。

一般的なAIコーディングエージェントが「コードの修正」自体を目的にしているのに対し、tomoeは**「生成されるMarkdownやログなどの成果物（Artifact）をLLM自身に評価させ、それを良くするために、発生源であるコードやプロンプトをチューニングする」**ことを目的としています。

また、AIが一度に多くの変更を加えてコードを破壊するのを防ぐため、**「1回のループにつき改善点はただ1つに絞る（便乗リファクタ禁止）」**という制約を設けているのが特徴です。
